...
// Adapt `Marshal` into a tuple item writer for the union StructuralUnion.
func writeStructuralUnionAsTupleItem(encoder *xml.Encoder, value *aastypes.StructuralUnion) error {
	return Marshal(encoder, value.Underlying(), false)
}

// Adapt `Marshal` into a tuple item writer for the union MixedUnion.
func writeMixedUnionAsTupleItem(encoder *xml.Encoder, value *aastypes.MixedUnion) error {
	return Marshal(encoder, value.Underlying(), false)
}

// Adapt `Marshal` into a tuple item writer for the union ModelTypedUnion.
func writeModelTypedUnionAsTupleItem(encoder *xml.Encoder, value *aastypes.ModelTypedUnion) error {
	return Marshal(encoder, value.Underlying(), false)
}
...
func writeSomethingAsSequence(
	...
	listStructuralPropertyUnderlying := make([]aastypes.IClass, len(that.ListStructuralProperty()))
	for i, item := range that.ListStructuralProperty() {
		listStructuralPropertyUnderlying[i] = item.Underlying()
	}
	err = writeListOfInstancesProperty(
		encoder,
		"listStructuralProperty",
		listStructuralPropertyUnderlying,
	)
	if err != nil {
		if seriaErr, ok := err.(*SerializationError); ok {
			seriaErr.Path.PrependName(
	...
	listMixedPropertyUnderlying := make([]aastypes.IClass, len(that.ListMixedProperty()))
	for i, item := range that.ListMixedProperty() {
		listMixedPropertyUnderlying[i] = item.Underlying()
	}
	err = writeListOfInstancesProperty(
		encoder,
		"listMixedProperty",
		listMixedPropertyUnderlying,
	)
	if err != nil {
		if seriaErr, ok := err.(*SerializationError); ok {
			seriaErr.Path.PrependName(
	...
	listModelTypedPropertyUnderlying := make([]aastypes.IClass, len(that.ListModelTypedProperty()))
	for i, item := range that.ListModelTypedProperty() {
		listModelTypedPropertyUnderlying[i] = item.Underlying()
	}
	err = writeListOfInstancesProperty(
		encoder,
		"listModelTypedProperty",
		listModelTypedPropertyUnderlying,
	)
	if err != nil {
		if seriaErr, ok := err.(*SerializationError); ok {
			seriaErr.Path.PrependName(
	...
	err = writeTuple3(
		encoder,
		that.TupleProperty(),
		writeStructuralUnionAsTupleItem,
		writeMixedUnionAsTupleItem,
		writeModelTypedUnionAsTupleItem,
	)
	if err != nil {
		return
	}
